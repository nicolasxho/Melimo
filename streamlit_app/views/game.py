    ...
    with col_grid:
        ...
        if feedback:
            if feedback["kind"] == "correct":
                st.success(f"✓ Correct !  **{feedback['word'].answer}**")
            elif feedback["kind"] == "mystery":
                st.success(f"🎉 MOT MYSTÈRE ! **{feedback['word'].answer}** — +2 points !")
                st.balloons()
            ...
            if submitted and user_input.strip():
                if check_answer(user_input, current_word):
                    ...
                    if current_word.is_mystery:
                        ...
                        st.session_state.last_feedback = {"kind": "mystery", "word": current_word}
                    else:
                        state.score += 1
                        st.session_state.last_feedback = {"kind": "correct", "word": current_word}
